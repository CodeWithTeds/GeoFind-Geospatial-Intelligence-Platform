...
<body class="h-screen w-screen overflow-hidden relative font-sans bg-black">
    ...
    <div id="game-map-container" class="fixed top-0 left-0 w-full h-full z-[1] bg-black">
        ...
        <a href="{{ request('level') ? route('levels') : route('dashboard') }}" id="exit-map-btn" aria-label="Close Map" class="absolute top-4 left-4 z-50 bg-black/50 text-white p-2 rounded-full backdrop-blur-sm border border-white/20 hover:bg-black/80 transition-colors">
