        ...
        <div class="accordion-body">
            <form id="convexHullForm">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select bg-white text-dark border-secondary" name="point_ids[]" multiple required>
                        @foreach ($locations as $location)
                            <option value="{{ $location->id }}"> {{ $location->name }}</option>
                        @endforeach
                    </select>
                    <div class="form-text text-muted">Hold Ctrl/Cmd to select multiple points</div>
                </div>
